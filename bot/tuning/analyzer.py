...
class PerformanceAnalyzer:
    ...
    def _analyze_by_market_condition(self, trades: list[TradeRecord]) -> list[dict]:
        ...
        conditions = {t.parameters.market_condition for t in trades}
        results = []
            ...
            results.append(
                {
                    ...
                    "win_rate_pct": (len(wins) / len(condition_trades)) * 100
                    if condition_trades
                    else 0,
                    "avg_pnl_usd": mean([t.pnl_usd for t in condition_trades])
                    if condition_trades
                    else 0,
                    "total_pnl_usd": sum(t.pnl_usd for t in condition_trades),
                    "avg_duration_seconds": mean([t.duration_seconds for t in condition_trades])
                    if condition_trades
                    else 0,
    ...
    def _analyze_parameter_effectiveness(self, trades: list[TradeRecord]) -> dict:
        ...
        def analyze_parameter_buckets(
            trades: list[TradeRecord], param_getter, bucket_size: float
        ) -> list[dict]:
            ...
            while current <= max_val:
                bucket_trades = [
                    t for t in trades if current <= param_getter(t) < current + bucket_size
                ]
                if bucket_trades:
    ...
    def _analyze_temporal_patterns(self, trades: list[TradeRecord]) -> dict:
        ...
        hourly_performance: dict[int, list[TradeRecord]] = {}
        for trade in trades:
