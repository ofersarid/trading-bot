...
class StatusBar(Horizontal):
    ...
    def update_strategy(
        ...
        """
        Update the right section with AI strategy info.

        Args:
            strategy_name: Current AI trading strategy name
            ai_enabled: Whether AI mode is enabled
            decision_interval: Seconds between AI decisions
        """
        if ai_enabled:
            status_str = f"[{COLOR_UP}]🤖 AI ACTIVE[/{COLOR_UP}]"
            strategy_content = (
                f"{status_str}  │  "
                f"Strategy: [cyan]{strategy_name}[/cyan]  │  "
                f"Decisions: [dim]every {decision_interval}s[/dim]  │  "
                f"[dim]Press S to change strategy[/dim]"
            )
        else:
            strategy_content = (
                "[yellow]📐 RULE-BASED[/yellow]  │  [dim]Press A to enable AI mode[/dim]"
            )

        try:
            bar = self.query_one("#threshold-bar", Static)
            bar.update(strategy_content)
        except Exception:
            pass
    ...
    def update_thresholds(
        ...
        """
        Update the right threshold section (legacy - kept for compatibility).
        """
        # Redirect to strategy display if needed
        pass
