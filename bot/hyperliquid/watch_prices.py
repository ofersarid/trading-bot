...
def watch_prices(coins: list[str] | None = None, interval: float = 1.0):
    """
        ...
        while True:
            ...
            for coin in coins:
                ...
                if prev is not None:
                    ...
                    print(
                        f"   {coin:6} ${price:>12,.2f}  {arrow} {sign}{change:>8,.2f} ({sign}{pct:.3f}%)"
                    )
                else:
...
if __name__ == "__main__":
    ...
    coins = [c.upper() for c in sys.argv[1:]] if len(sys.argv) > 1 else ["BTC", "ETH", "SOL"]
