...
class SessionStateManager:
    ...
    def _migrate_old_sessions(self) -> None:
        ...
        if (
            old_state_file.exists()
            and self.session_name == self.DEFAULT_SESSION
        ...
        if old_flat_file.exists() and old_flat_file.is_file() and not self.state_file.exists():
    ...
    def list_sessions(cls, data_dir: str = "data") -> list[dict]:
        ...
        for session_folder in sorted(sessions_dir.iterdir()):
            ...
            try:
                ...
                has_report = (
                    (reports_dir / "report.json").exists() if reports_dir.exists() else False
                )
